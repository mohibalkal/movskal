...
const TabContent = ({ type, viewMode, sortBy, genreFilter, platformFilters }: TabContentProps) => {
  ...
  if (showsQuery.isError) {
    return <div className="py-12 text-center text-white">{t('tvShows.error', 'Error loading TV shows. Please try again.')}</div>;
  }
  ...
  const title = type === 'popular' 
    ? t('tvShows.sections.popular', 'Popular TV Shows') 
    : type === 'top_rated' 
      ? t('tvShows.sections.topRated', 'Top Rated TV Shows') 
      : t('tvShows.sections.trending', 'Trending TV Shows');
