  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-white">{t('tv.seasonsAndEpisodes', 'Seasons & Episodes')}</h2>
        <div className="text-sm text-white/90 px-3 py-1 bg-black/30 rounded-full border border-white/10">
          {episodes.length} {t('tv.episodes', 'episodes')}
        </div>
      </div>
      
      <div className="relative mb-8 bg-black/40 backdrop-blur-sm rounded-lg p-4 border border-white/5 shadow-lg">
        <div className="text-sm text-white font-medium mb-4">{t('tv.selectSeason', 'Select Season:')}</div>
          ...
          {!showAllSeasons && (
            <>
              <button 
                ...
                aria-label={t('tv.prevSeasons', 'Previous seasons')}
              >
              ...
              <button 
                ...
                aria-label={t('tv.nextSeasons', 'Next seasons')}
              >
                ...
                >
                  <div className="w-full bg-black/60 border-b border-white/5 py-1.5 px-3 text-center">
                    <span className="text-sm font-medium text-white/90">
                      {t('tv.season', 'Season')} {season.season_number}
                    </span>
                  </div>
                  ...
                  <div className="p-3 flex flex-col items-center w-full">
                    ...
                    <div className="text-center">
                      <span className="text-xs text-white/70 block">{season.episode_count || 0} {t('tv.episodes', 'episodes')}</span>
                      
                      {progress > 0 && progress < 100 && (
                        <span className="text-xs bg-black/50 border border-white/10 rounded-full px-2 py-0.5 inline-block mt-1 text-white/90">
                          {progress}% {t('tv.watched', 'watched')}
                        </span>
                      )}
                    </div>
                  </div>
        ...
        {filteredSeasons.length > 0 && (
          <div className="mt-6 pt-4 border-t border-white/10 text-sm text-white/80">
            <div className="flex items-center gap-2">
              <span className="text-white font-medium">{t('tv.season', 'Season')} {selectedSeason}</span>
              {watchProgress[selectedSeason] > 0 && (
                <span className="px-2 py-0.5 rounded-full bg-accent/80 text-xs text-white">
                  {watchProgress[selectedSeason]}% {t('tv.watched', 'watched')}
                </span>
              )}
            </div>
            {
      ...
      <h3 className="text-xl font-medium text-white mb-4">{t('tv.episodesSection', 'Episodes')}</h3>
        ...
        {episodes.length > 0 ? (
          episodes.map(episode => (
            <div key={episode.id} className="bg-black/50 backdrop-blur-sm border border-white/5 overflow-hidden rounded-lg flex flex-col h-full shadow-md">
              <div className="relative">
                {episode.still_path && getImageUrl(episode.still_path, backdropSizes.small) ? (
                  <img 
                ...
                <div className={`w-full h-48 bg-black/60 flex items-center justify-center ${episode.still_path && getImageUrl(episode.still_path, backdropSizes.small) ? 'hidden' : ''}`}>
                  <span className="text-white/30">{t('tv.noImage', 'No image available')}</span>
                </div>
                ...
                <div className="absolute top-2 right-2 bg-black/70 border border-white/10 rounded-full px-2 py-1 text-xs font-medium text-white/90">
                  {t('tv.episode', 'Episode')} {episode.episode_number}
                </div>
              ...
              <div className="p-4 flex-1 flex flex-col">
                <div className="flex-1">
                  ...
                  <p className="text-white/70 text-sm mb-3 line-clamp-3">{episode.overview || t('tv.noOverview', 'No overview available.')}</p>
                </div>
          ...
          <div className="text-center py-8 text-white/80 col-span-2 bg-black/50 backdrop-blur-sm border border-white/5 rounded-lg shadow-md">
            <p>{t('tv.noEpisodesForSeason', 'No episodes available for Season {{season}}.', { season: selectedSeason })}</p>
            {filteredSeasons.length > 0 && selectedSeason !== filteredSeasons[0].season_number && (
              ...
              >
                {t('tv.viewSeason', 'View Season {{season}}', { season: filteredSeasons[0].season_number })}
              </Button>
            )}
          </div>
        )}
