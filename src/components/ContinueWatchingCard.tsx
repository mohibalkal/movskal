...
const formatLastWatched = (dateString: string, fallback: string) => {
...
const formatTimeRemaining = (position: number, duration: number, suffix: string) => {
  if (!duration) return '';
      ...
      <div className="absolute bottom-4 left-4 right-4 z-10">
          ...
          <TooltipProvider delayDuration={300}>
            <Tooltip>
              ...
              <TooltipContent side="top">
                <p>{t('home.continueWatching.viewDetails', 'View details')}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        <div className="flex items-center justify-between text-xs text-white/70 mb-2">
          <span className="flex items-center">
            ...
            {formatLastWatched(item.created_at, t('home.continueWatching.recently', 'Recently'))}
          </span>
          {item.media_type === 'tv' && (
        ...
        <div className="mb-3 relative">
          ...
          <div className="text-xs text-white/70 mt-1 text-right">
            {formatTimeRemaining(item.watch_position, item.duration, t('home.continueWatching.remaining', 'remaining'))}
          </div>
        </div>
        <Button
        ...
        >
          ...
          {t('home.continueWatching.continue', 'Continue')}
        </Button>
      </div>
    </motion.div>
  );
