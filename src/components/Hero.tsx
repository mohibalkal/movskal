...
const Hero = ({ media, className = '' }: HeroProps) => {
  ...
  const preloadImage = useCallback((backdrop_path: string) => {
    ...
    // Check if image is already loaded to avoid duplicate preloads
    const imgUrl = getImageUrl(backdrop_path, backdropSizes.medium);
    if (document.querySelector(`link[href="${imgUrl}"]`)) {
      return;
    }
    ...
    img.onload = () => {
      console.log('Successfully preloaded image:', imgUrl);
    };
  }, [buildSrcSet]);
  ...
  useEffect(() => {
    if (isLoaded) {
      // Add a small delay to ensure the current image is fully loaded
  ...
  useEffect(() => {
    if (currentIndex === 0 && featuredMedia?.backdrop_path && isLoaded) {
      ...
      // Add a timeout to ensure the image is used within a reasonable time
      const timeoutId = setTimeout(() => {
  ...
  }, [currentIndex, featuredMedia, buildSrcSet, isLoaded]);
      ...
      >
        ...
        <div className="flex flex-wrap gap-3 md:gap-4">
          ...
          >
            ...
            <span className="font-medium">{t('home.hero.watch', 'Watch')}</span>
          </Button>
          ...
          >
            ...
            <span className="font-medium">{t('home.hero.moreInfo', 'More Info')}</span>
          </Button>
        </div>
      </motion.div>
    </div>
