...
export const DownloadSection: React.FC<DownloadSectionProps> = ({
  ...
  // Auto trigger fetching when requested (used when button is moved to header)
  React.useEffect(() => {
    if (autoFetch && !isLoading && downloadLinks.length === 0) {
      fetchLinks();
    }
    ...
    return (
      <div className="flex flex-col items-center gap-4 p-6">
        <Button onClick={fetchLinks} variant="outline">
          ...
          {t('media.showDownloadLinks', 'Show Download Links')}
        </Button>
      </div>
    );
  }
